...
void CHud::OnRender()
{
	...
	// CLIENT 14 Aimbot FOV Visual + Target Box
	if(!GameClient()->m_Snap.m_SpecInfo.m_Active && GameClient()->m_Snap.m_pLocalCharacter)
	{
		...
		vec2 CameraPos = GameClient()->m_Camera.m_Center;
		float PlayerScreenX = CenterX + (LocalPos.x - CameraPos.x) * Zoom;
		float PlayerScreenY = CenterY + (LocalPos.y - CameraPos.y) * Zoom;
		...
		if(g_Config.m_ClAimbot && g_Config.m_ClAimbotFov < 360)
		{
			...
			Graphics()->TextureClear();
			Graphics()->SetColor(1.0f, 0.3f, 0.3f, 0.5f);

			for(int s = -1; s <= 1; s += 2)
			{
				float a = MouseAngle + FovOffset * s;
				vec2 exDir = normalize(direction(a));
				vec2 initPos = LocalPos;
				vec2 finishPos = initPos + exDir * GameClient()->m_aTuning[g_Config.m_ClDummy].m_HookLength;
				vec2 colPos = finishPos;
				int teleNr = 0;
				GameClient()->Collision()->IntersectLineTeleHook(initPos, finishPos, &colPos, nullptr, &teleNr);

				float sx1 = PlayerScreenX;
				float sy1 = PlayerScreenY;
				float sx2 = CenterX + (colPos.x - CameraPos.x) * Zoom;
				float sy2 = CenterY + (colPos.y - CameraPos.y) * Zoom;

				// Draw 3 lines with offset for thickness
				Graphics()->LinesBegin();
				for(int off = -1; off <= 1; off++)
				{
					float ox = off * 0.5f;
					float oy = off * 0.5f;
					IGraphics::CLineItem L(vec2(sx1 + ox, sy1 + oy), vec2(sx2 + ox, sy2 + oy));
		...
		// Target Box - use predicted position
		if(g_Config.m_ClAimbot && GameClient()->m_C14Aimbot.TargetId() >= 0)
		{
			int TId = GameClient()->m_C14Aimbot.TargetId();
			vec2 TargetPos = GameClient()->m_aClients[TId].m_Predicted.m_Pos;
			float Tx = CenterX + (TargetPos.x - CameraPos.x) * Zoom;
			float Ty = CenterY + (TargetPos.y - CameraPos.y) * Zoom;
			float Bs = 14.0f;

			Graphics()->TextureClear();
			Graphics()->SetColor(1.0f, 0.2f, 0.2f, 0.9f);

			// Draw box with 3 passes for thickness
			for(int pass = -1; pass <= 1; pass++)
			{
				float o = pass * 0.5f;
				Graphics()->LinesBegin();
				IGraphics::CLineItem T1(vec2(Tx - Bs + o, Ty - Bs), vec2(Tx + Bs + o, Ty - Bs));
				IGraphics::CLineItem T2(vec2(Tx + Bs, Ty - Bs + o), vec2(Tx + Bs, Ty + Bs + o));
				IGraphics::CLineItem T3(vec2(Tx + Bs + o, Ty + Bs), vec2(Tx - Bs + o, Ty + Bs));
				IGraphics::CLineItem T4(vec2(Tx - Bs, Ty + Bs + o), vec2(Tx - Bs, Ty - Bs + o));
				Graphics()->LinesDraw(&T1, 1);
				Graphics()->LinesDraw(&T2, 1);
				Graphics()->LinesDraw(&T3, 1);
				Graphics()->LinesDraw(&T4, 1);
				Graphics()->LinesEnd();
			}
		}
	}

	// CLIENT 14 Active Settings HUD Overlay
	if(!GameClient()->m_Snap.m_SpecInfo.m_Active)
	{
		int ActiveCount = 0;
		if(g_Config.m_ClAimbot) ActiveCount++;
		if(g_Config.m_ClQuickStop) ActiveCount++;
		if(g_Config.m_ClAntiFreeze) ActiveCount++;
		if(g_Config.m_ClAvoidFreeze) ActiveCount++;
		if(g_Config.m_ClAutoJumpSave) ActiveCount++;
		if(g_Config.m_ClAutoHook) ActiveCount++;
		if(g_Config.m_ClHammerBot) ActiveCount++;
		if(g_Config.m_ClFreezeUnfreeze) ActiveCount++;
		if(g_Config.m_ClBalanceBot) ActiveCount++;
		if(g_Config.m_ClTransIn) ActiveCount++;
		if(g_Config.m_ClTransOut) ActiveCount++;

		if(ActiveCount > 0)
		{
			float BoxX = m_Width - 100.0f;
			float BoxY = 70.0f;
			float LineH = 9.0f;
			float BoxH = 6.0f + ActiveCount * LineH;

			Graphics()->TextureClear();
			Graphics()->DrawRect(BoxX - 3.0f, BoxY - 3.0f, 100.0f, BoxH, ColorRGBA(0.0f, 0.0f, 0.0f, 0.4f), IGraphics::CORNER_ALL, 3.0f);

			float YPos = BoxY;
			struct C14F { const char *pN; int On; float r, g, b; };
			C14F aF[] = {
				{"Aimbot", g_Config.m_ClAimbot, 1.0f, 0.3f, 0.3f},
				{"Quick Stop", g_Config.m_ClQuickStop, 0.3f, 1.0f, 0.3f},
				{"Anti-Freeze", g_Config.m_ClAntiFreeze, 0.3f, 0.7f, 1.0f},
				{"Avoid Freeze", g_Config.m_ClAvoidFreeze, 1.0f, 1.0f, 0.3f},
				{"Auto Jump", g_Config.m_ClAutoJumpSave, 1.0f, 0.6f, 0.2f},
				{"Auto Hook", g_Config.m_ClAutoHook, 0.3f, 1.0f, 0.7f},
				{"Hammer Bot", g_Config.m_ClHammerBot, 1.0f, 0.5f, 0.3f},
				{"Freeze Unfreeze", g_Config.m_ClFreezeUnfreeze, 0.5f, 0.8f, 1.0f},
				{"Balance Bot", g_Config.m_ClBalanceBot, 0.8f, 0.4f, 1.0f},
				{"Trans In", g_Config.m_ClTransIn, 0.45f, 0.45f, 1.0f},
				{"Trans Out", g_Config.m_ClTransOut, 0.45f, 0.45f, 1.0f},
			};
			...
			for(int i = 0; i < 11; i++)
			{
				if(aF[i].On)
				{
					TextRender()->TextColor(aF[i].r, aF[i].g, aF[i].b, 1.0f);
					TextRender()->Text(BoxX, YPos, 8.0f, aF[i].pN, -1.0f);
					YPos += LineH;
				}
			}
			TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);
