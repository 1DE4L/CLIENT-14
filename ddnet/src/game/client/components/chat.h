...
class CChat : public CComponent
{
	...
	struct CTransPending
	{
		...
		bool m_Incoming;
		bool m_OriginalSent;
		char m_aOriginal[512];
		char m_aFullLine[768];
		char m_aMentionedName[64];
		char m_aSrcLang[8];
		char m_aDstLang[8];
	};
	std::vector<CTransPending> m_aTransPending;
	std::unordered_map<std::string, std::string> m_TransCache;
	int64_t m_LastTransRequestTime = 0;
	void RequestTranslation(int ClientId, int Team, const char *pText, const char *pSrcLang, const char *pDstLang, bool Incoming, const char *pMentionedName, bool OriginalSent, const char *pFullLine);
	void ProcessTranslations();
	const char *PrepareMsgForTrans(const char *pMessage, char *pNameBuf, int NameBufSize) const;
...
public:
	...
	void SendChat(int Team, const char *pLine, bool NoTrans = false);
