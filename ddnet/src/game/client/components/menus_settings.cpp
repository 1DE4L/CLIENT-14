...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	else if(g_Config.m_UiSettingsPage == SETTINGS_CLIENT14)
	{
		GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_CLIENT14);
		RenderSettingsClient14(MainView);
	}
	else
	{
	...
	if(NeedRestart)
	{
		...
		if(m_NeedRestartUpdate)
		{
			...
			Ui()->DoLabel(&RestartWarning, Localize("CLIENT 14 needs to be restarted to complete update!"), 14.0f, TEXTALIGN_ML);
...
void CMenus::RenderSettingsClient14(CUIRect MainView)
{
	...
	Left.HSplitTop(20.0f, &Button, &Left);
	if(DoButton_CheckBox(&g_Config.m_ClAimbot, "Enable Aimbot", g_Config.m_ClAimbot, &Button))
		g_Config.m_ClAimbot ^= 1;

	Left.HSplitTop(20.0f, &Button, &Left);
	Ui()->DoScrollbarOption(&g_Config.m_ClAimbotFov, &g_Config.m_ClAimbotFov, &Button, "FOV", 5, 180, &CUi::ms_LinearScrollbarScale, 0u, " degrees");

	Left.HSplitTop(20.0f, &Button, &Left);
	if(DoButton_CheckBox(&g_Config.m_ClAimbotSilent, "Silent Aimbot", g_Config.m_ClAimbotSilent, &Button))
		g_Config.m_ClAimbotSilent ^= 1;

	Left.HSplitTop(20.0f, &Button, &Left);
	Ui()->DoScrollbarOption(&g_Config.m_ClAimbotTarget, &g_Config.m_ClAimbotTarget, &Button, "Target Priority", 0, 1, &CUi::ms_LinearScrollbarScale, 0u, g_Config.m_ClAimbotTarget == 0 ? " Crosshair" : " Player");
	...
	Right.HSplitTop(20.0f, &Button, &Right);
	Ui()->DoScrollbarOption(&g_Config.m_ClPlayerTracker, &g_Config.m_ClPlayerTracker, &Button, "Tracker Mode", 0, 2, &CUi::ms_LinearScrollbarScale, 0u, g_Config.m_ClPlayerTracker == 0 ? "Off" : g_Config.m_ClPlayerTracker == 1 ? "War" : "All");
	...
	static const char *apCommands[10] = {
		...
		"toggle cl_quick_stop 0 1",
		"toggle cl_avoid_freeze 0 1",
		"toggle cl_anti_freeze 0 1",
		"toggle cl_auto_jump_save 0 1",
		"toggle cl_balance_bot 0 1",
		"+auto_hook",
		"toggle cl_hammer_bot 0 1",
		"toggle cl_freeze_unfreeze 0 1",
		"+auto_fire",
	};
	static const char *apLabels[10] = {
		"Aimbot",
		"Quick Stop",
		"Avoid Freeze",
		"Anti-Freeze",
	...
	for(int i = 0; i < 10; i++)
	{
		...
		// Find current binding for this command
		CBindSlot CurrentBind = EMPTY_BIND_SLOT;
		for(int k = 0; k < KEY_LAST; k++)
		{
			for(int m = 0; m < KeyModifier::COMBINATION_COUNT; m++)
			{
				const char *pBindStr = GameClient()->m_Binds.Get(k, m);
				if(pBindStr && str_comp(pBindStr, apCommands[i]) == 0)
				{
					CurrentBind = CBindSlot(k, m);
					break;
				}
			}
			if(CurrentBind != EMPTY_BIND_SLOT)
				break;
		}

		// Show key reader button
		CKeyBinder::CKeyReaderResult Result = GameClient()->m_KeyBinder.DoKeyReader(
			&m_aClient14BindButtons[i * 2],
			&m_aClient14BindButtons[i * 2 + 1],
			&KeyRect,
			CurrentBind,
			m_aClient14BindActive[i]
		);

		if(m_aClient14BindActive[i] && Result.m_Bind != EMPTY_BIND_SLOT)
			m_aClient14BindActive[i] = false;

		if(Result.m_Aborted)
		{
			m_aClient14BindActive[i] = false;
		}
		else if(Result.m_Bind != CurrentBind)
		{
			// Unbind old key
			if(CurrentBind.m_Key != KEY_UNKNOWN || Result.m_Bind.m_Key == KEY_UNKNOWN)
			{
			...
			{
				GameClient()->m_Binds.Bind(Result.m_Bind.m_Key, apCommands[i], false, Result.m_Bind.m_ModifierMask);
			}
		}
	}
}
...
void CMenus::RenderSettingsDDNet(CUIRect MainView)
{
...
#if defined(CONF_AUTOUPDATE)
	{
		...
		{
			str_format(aBuf, sizeof(aBuf), Localize("CLIENT 14 %s is available:"), Client()->LatestVersion());
		...
		{
			str_copy(aBuf, Localize("CLIENT 14 updated!"));
