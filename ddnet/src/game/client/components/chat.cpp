...
void CChat::Reset()
{
	...
	// CLIENT 14: Clear pending chat translations on reset to prevent cross-server leaks
	m_aTransPending.clear();
	m_TransCache.clear();
	m_LastTransRequestTime = 0;
...
void CChat::OnMessage(int MsgType, void *pRawMsg)
{
	...
	if(MsgType == NETMSGTYPE_SV_CHAT)
	{
		...
		*/
		...
		// CLIENT 14 Incoming Chat Translation
		if(g_Config.m_ClTransIn && pMsg->m_ClientId >= 0 && pMsg->m_ClientId != GameClient()->m_Snap.m_LocalClientId)
		{
			char aMentionedName[64];
			const char *pTransStart = PrepareMsgForTrans(pMsg->m_pMessage, aMentionedName, sizeof(aMentionedName));
			if(str_length(pTransStart) > 1)
			{
				RequestTranslation(pMsg->m_ClientId, pMsg->m_Team, pTransStart,
					g_Config.m_ClTransInSrc, g_Config.m_ClTransInDst, true, aMentionedName, true, pMsg->m_pMessage);
			}
		}
...
// CLIENT 14 Chat Translator
const char *CChat::PrepareMsgForTrans(const char *pMessage, char *pNameBuf, int NameBufSize) const
{
	...
	if(pColonAt)
	{
		...
		if(PrefixLen < NameBufSize)
		{
			for(int i = 0; i < MAX_CLIENTS; i++)
			{
				if(GameClient()->m_aClients[i].m_Active &&
				   str_comp_num(GameClient()->m_aClients[i].m_aName, pMessage, PrefixLen) == 0)
				{
					pTranslateStart = pColonAt + 1;
					str_copy(pNameBuf, pMessage, std::min(NameBufSize, PrefixLen + 1));
...
void CChat::RequestTranslation(int ClientId, int Team, const char *pText, const char *pSrcLang, const char *pDstLang, bool Incoming, const char *pMentionedName, bool OriginalSent, const char *pFullLine)
{
	// CLIENT 14: Cache lookup — skip HTTP for repeated translations.
	if(g_Config.m_ClTransCacheSize > 0)
	{
		char aKey[1400];
		str_format(aKey, sizeof(aKey), "%s|%s|%s", pSrcLang, pDstLang, pText);
		auto It = m_TransCache.find(aKey);
		if(It != m_TransCache.end())
		{
			const char *pTranslated = It->second.c_str();
			if(pTranslated[0] != '\0' && str_comp_nocase(pText, pTranslated) != 0)
			{
				if(Incoming)
				{
					char aBuf[1024];
					str_format(aBuf, sizeof(aBuf), "%s%s%s (%s→%s)",
						pMentionedName, pMentionedName[0] != '\0' ? ": " : "", pTranslated,
						pSrcLang, pDstLang);
					AddLine(ClientId, MODE_TRANS, aBuf);
				}
				else
				{
					char aBuf[512];
					if(pMentionedName[0] != '\0')
						str_format(aBuf, sizeof(aBuf), "%s: %s", pMentionedName, pTranslated);
					else
						str_copy(aBuf, pTranslated, sizeof(aBuf));
					SendChat(Team, aBuf, true);
				}
			}
			return;
		}
	...
	// CLIENT 14: Rate limit + pending cap to avoid flooding the public API.
	const int64_t Now = time();
	const int64_t MinIntervalTicks = (int64_t)g_Config.m_ClTransRateLimitMs * time_freq() / 1000;
	const bool RateLimited = g_Config.m_ClTransRateLimitMs > 0 && (Now - m_LastTransRequestTime) < MinIntervalTicks;
	const bool TooManyPending = (int)m_aTransPending.size() >= 8;

	if(RateLimited || TooManyPending)
	{
		// Outgoing: never lose the message — fall back to sending the original.
		if(!Incoming && !OriginalSent && pFullLine && str_length(pFullLine) > 0)
			SendChat(Team, pFullLine, true);
		return;
	}
	...
	char aUrl[2048];
	if(g_Config.m_ClTransApiEmail[0] != '\0')
	{
		char aEscapedEmail[128];
		EscapeUrl(aEscapedEmail, sizeof(aEscapedEmail), g_Config.m_ClTransApiEmail);
		str_format(aUrl, sizeof(aUrl), "https://api.mymemory.translated.net/get?q=%s&langpair=%s|%s&de=%s", aEscaped, pSrcLang, pDstLang, aEscapedEmail);
	}
	else
	{
		str_format(aUrl, sizeof(aUrl), "https://api.mymemory.translated.net/get?q=%s&langpair=%s|%s", aEscaped, pSrcLang, pDstLang);
	}

	std::unique_ptr<CHttpRequest> pReqUnique = HttpGet(aUrl);
	pReqUnique->WriteToMemory();
	pReqUnique->FailOnErrorStatus(false);
	pReqUnique->Timeout(CTimeout{5000, 10000, 500, 5});
	std::shared_ptr<CHttpRequest> pRequest = std::move(pReqUnique);
	Http()->Run(std::static_pointer_cast<IHttpRequest>(pRequest));

	m_LastTransRequestTime = Now;
	...
	Entry.m_OriginalSent = OriginalSent;
	str_copy(Entry.m_aOriginal, pText, sizeof(Entry.m_aOriginal));
	str_copy(Entry.m_aFullLine, pFullLine ? pFullLine : pText, sizeof(Entry.m_aFullLine));
	str_copy(Entry.m_aMentionedName, pMentionedName ? pMentionedName : "", sizeof(Entry.m_aMentionedName));
	str_copy(Entry.m_aSrcLang, pSrcLang, sizeof(Entry.m_aSrcLang));
	str_copy(Entry.m_aDstLang, pDstLang, sizeof(Entry.m_aDstLang));
	m_aTransPending.push_back(Entry);
}

void CChat::ProcessTranslations()
{
	for(auto it = m_aTransPending.begin(); it != m_aTransPending.end(); )
	{
		if(it->m_pRequest->Done())
		{
			EHttpState State = it->m_pRequest->State();
			if(State == EHttpState::DONE)
			{
				json_value *pJson = it->m_pRequest->ResultJson();
				if(pJson)
				{
					const char *pTranslated = (*pJson)["responseData"]["translatedText"];
					if(pTranslated && str_length(pTranslated) > 0 && str_comp_nocase(it->m_aOriginal, pTranslated) != 0)
					{
						// CLIENT 14: Store successful translation in cache.
						if(g_Config.m_ClTransCacheSize > 0)
						{
							if((int)m_TransCache.size() >= g_Config.m_ClTransCacheSize)
								m_TransCache.clear();
							char aKey[1400];
							str_format(aKey, sizeof(aKey), "%s|%s|%s", it->m_aSrcLang, it->m_aDstLang, it->m_aOriginal);
							m_TransCache[aKey] = pTranslated;
						}
						if(it->m_Incoming)
						{
							char aBuf[1024];
							str_format(aBuf, sizeof(aBuf), "%s%s%s (%s→%s)",
								it->m_aMentionedName, it->m_aMentionedName[0] != '\0' ? ": " : "", pTranslated,
								g_Config.m_ClTransInSrc, g_Config.m_ClTransInDst);
							AddLine(it->m_ClientId, MODE_TRANS, aBuf);
						}
						else
						{
							char aBuf[512];
							if(it->m_aMentionedName[0] != '\0')
								str_format(aBuf, sizeof(aBuf), "%s: %s", it->m_aMentionedName, pTranslated);
							else
								str_copy(aBuf, pTranslated, sizeof(aBuf));
							SendChat(it->m_Team, aBuf, true);
						}
					}
			...
			// CLIENT 14: For outgoing messages where the original was not sent, fall back
			// to sending the original on any non-success state so the message isn't lost.
			if(!it->m_Incoming && !it->m_OriginalSent && str_length(it->m_aFullLine) > 0)
			{
				if(State != EHttpState::DONE)
				{
					SendChat(it->m_Team, it->m_aFullLine, true);
				}
				else
				{
					// Translation returned but we never sent the original — send it now
					// as a follow-up so the recipient sees both the original and translation.
					SendChat(it->m_Team, it->m_aFullLine, true);
				}
			}

			it = m_aTransPending.erase(it);
		}
		else
		{
			++it;
...
{
	if(ClientId >= 0)
		GameClient()->SetLastChatMsg(ClientId, pLine);
	...
	CurrentLine.m_Whisper = (Team == 2 || Team == 3);
	...
	else if(Highlighted && Client()->State() != IClient::STATE_DEMOPLAYBACK)
	{
		if(Now - m_aLastSoundPlayed[CHAT_HIGHLIGHT] >= time_freq() * 3 / 10)
		{
			...
			Client()->Notify("CLIENT 14 Chat", aBuf);
...
void CChat::OnPrepareLines(float y)
{
	...
	for(int i = 0; i < MAX_LINES; i++)
	{
		...
		else if(Line.m_TeamNumber == MODE_TRANS)
			NameColor = ColorRGBA(0.3f, 0.3f, 0.9f, 1.0f);
		else if(Line.m_Team)
		...
		if(Line.m_CustomColor)
			...
			Color = ColorRGBA(0.45f, 0.45f, 1.0f, 1.0f);
...
void CChat::OnRender()
{
	...
	// CLIENT 14 process chat translations
	ProcessTranslations();
...
void CChat::SendChat(int Team, const char *pLine, bool NoTrans)
{
...
void CChat::SendChatQueued(const char *pLine)
{
	...
	// CLIENT 14 Outgoing Chat Translation
	bool DidTransQueue = false;
	bool OriginalSent = true;
	if(g_Config.m_ClTransOut && pLine[0] != '/')
	{
		char aMentionedName[64];
		const char *pTransStart = PrepareMsgForTrans(pLine, aMentionedName, sizeof(aMentionedName));
		if(str_length(pTransStart) > 1)
		{
			// If show_original is on, we will send the original now. Otherwise the original
			// waits for the translation response (and is sent as fallback if the request fails).
			OriginalSent = g_Config.m_ClTransShowOriginal != 0;
			RequestTranslation(-2, m_Mode == MODE_ALL ? 0 : 1, pTransStart,
				g_Config.m_ClTransOutSrc, g_Config.m_ClTransOutDst, false, aMentionedName, OriginalSent, pLine);
			DidTransQueue = true;
		}
	}
	...
	{
		if(!DidTransQueue || g_Config.m_ClTransShowOriginal)
	...
	else if(m_PendingChatCounter < 3)
	{
		if(!DidTransQueue || g_Config.m_ClTransShowOriginal)
			...
			DidTransQueue = false; // don't count translation-only as pending
		AddEntry = true;
	}
