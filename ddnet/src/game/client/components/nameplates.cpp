...
void CNamePlates::RenderNamePlateGame(vec2 Position, const CNetObj_PlayerInfo *pPlayerInfo, float Alpha)
{
	...
	if(GameClient()->IsWarPlayer(ClientData.m_aName))
	{
		Data.m_Color = ColorRGBA(1.0f, 0.25f, 0.25f);
	}
	else if(g_Config.m_ClNamePlatesTeamcolors)
	{
	...
	if(g_Config.m_ClChatBalloons && pPlayerInfo->m_ClientId >= 0)
	{
		int64_t Time;
		const char *pMsg = GameClient()->GetLastChatMsg(pPlayerInfo->m_ClientId, &Time);
		int64_t Elapsed = time_get() - Time;
		int64_t Duration = g_Config.m_ClChatBalloonsTime * time_freq();
		if(pMsg && pMsg[0] != '\0' && Elapsed < Duration)
		{
			float BalloonAlpha = 1.0f - (float)Elapsed / (float)Duration;
			BalloonAlpha *= Alpha;
			...
			Cursor.m_Flags = TEXTFLAG_RENDER | TEXTALIGN_BC;
			...
			float TextWidth = Box.m_W;
			float TextHeight = Box.m_H;
			float OffsetY = (float)g_Config.m_ClNamePlatesOffset + 20.0f;
			vec2 BalloonPos = Position - vec2(0.0f, OffsetY);
			ColorRGBA BalloonColor(1.0f, 1.0f, 1.0f, BalloonAlpha);
			ColorRGBA OutlineColor(0.0f, 0.0f, 0.0f, 0.5f * BalloonAlpha);
			TextRender()->RenderTextContainer(Idx, BalloonColor, OutlineColor, BalloonPos.x - TextWidth / 2.0f, BalloonPos.y - TextHeight - 5.0f);
...
void CNamePlates::OnRender()
{
...
#endif
	const bool RenderNameplates = g_Config.m_ClNamePlates || ShowDirection != 0;
	const bool RenderTracker = g_Config.m_ClPlayerTracker > 0;

	if(!RenderNameplates && !RenderTracker)
		return;

	const float ScreenW = (float)Graphics()->ScreenWidth();
	const float ScreenH = (float)Graphics()->ScreenHeight();
	const float ScreenX0 = ScreenW * -0.3f;
	const float ScreenY0 = ScreenH * -0.3f;
	const float ScreenX1 = ScreenW * 1.3f;
	const float ScreenY1 = ScreenH * 1.3f;
	const float EdgeMargin = g_Config.m_ClPlayerTrackerSize * 0.5f;
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(RenderNameplates && GameClient()->m_aClients[i].m_SpecCharPresent)
		{
		...
		if(RenderNameplates && GameClient()->m_Snap.m_aCharacters[i].m_Active)
		{
			const vec2 RenderPos = GameClient()->m_aClients[i].m_RenderPos;
			RenderNamePlateGame(RenderPos, pInfo, 1.0f);
		}

		if(RenderTracker && pInfo->m_ClientId >= 0 && !pInfo->m_Local && GameClient()->m_Snap.m_aCharacters[i].m_Active)
		{
			if(g_Config.m_ClPlayerTracker == 1 && !GameClient()->IsWarPlayer(GameClient()->m_aClients[i].m_aName))
				continue;

			const vec2 RenderPos = GameClient()->m_aClients[i].m_RenderPos;
			bool OnScreen = in_range(RenderPos.x, ScreenX0, ScreenX1) && in_range(RenderPos.y, ScreenY0, ScreenY1);

			if(!OnScreen)
			{
				vec2 ScreenCenter(ScreenW * 0.5f, ScreenH * 0.5f);
				vec2 Dir = RenderPos - ScreenCenter;
				...
				float YRatio = 1.0f;
				if(Dir.x > 0.0f)
					XRatio = (ScreenW * 0.5f - EdgeMargin) / Dir.x;
				else if(Dir.x < 0.0f)
					XRatio = (-ScreenW * 0.5f + EdgeMargin) / Dir.x;
				if(Dir.y > 0.0f)
					YRatio = (ScreenH * 0.5f - EdgeMargin) / Dir.y;
				else if(Dir.y < 0.0f)
					YRatio = (-ScreenH * 0.5f + EdgeMargin) / Dir.y;
				float Ratio = minimum(XRatio, YRatio);
				if(Ratio <= 0.0f || Ratio > 1000.0f)
					continue;

				vec2 EdgePos = ScreenCenter + Dir * Ratio;
				float FontSize = g_Config.m_ClPlayerTrackerSize * 0.3f;

				char aText[48];
				int Dist = (int)(Len / 32.0f);
				str_format(aText, sizeof(aText), "%s (%d)", GameClient()->m_aClients[i].m_aName, Dist);

				STextContainerIndex TextIdx;
				CTextCursor Cursor;
				Cursor.m_FontSize = FontSize;
				Cursor.m_Flags = TEXTFLAG_RENDER | TEXTALIGN_MC;
				TextRender()->CreateTextContainer(TextIdx, &Cursor, aText);
				STextBoundingBox Box = TextRender()->GetBoundingBoxTextContainer(TextIdx);

				ColorRGBA TrackColor(1.0f, 0.35f, 0.35f, 0.85f);
			float TextX = EdgePos.x - Box.m_W * 0.5f;
			float TextY = EdgePos.y - Box.m_H * 0.5f;
			TextX = TextX < EdgeMargin ? EdgeMargin : (TextX > ScreenW - Box.m_W - EdgeMargin ? ScreenW - Box.m_W - EdgeMargin : TextX);
			TextY = TextY < EdgeMargin ? EdgeMargin : (TextY > ScreenH - Box.m_H - EdgeMargin ? ScreenH - Box.m_H - EdgeMargin : TextY);

				TextRender()->RenderTextContainer(TextIdx, TrackColor, ColorRGBA(0, 0, 0, 0.5f), TextX, TextY);
				TextRender()->DeleteTextContainer(TextIdx);
			}
		}
	}
}
