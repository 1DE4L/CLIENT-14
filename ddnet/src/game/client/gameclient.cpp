...
void CGameClient::OnConsoleInit()
{
	...
	Console()->Register("war", "s[add|remove|list] ?r[name]", CFGFLAG_CLIENT, ConWar, this, "Manage war player list. Usage: war add <name>, war remove <name>, war list");
...
void CGameClient::OnInit()
{
	...
	const char *pLoadingDDNetCaption = Localize("Loading CLIENT 14");
...
void CGameClient::ConWar(IConsole::IResult *pResult, void *pUserData)
{
	CGameClient *pClient = static_cast<CGameClient *>(pUserData);
	char aBuf[512];
	if(pResult->NumArguments() == 0)
	{
		pClient->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "war", "Usage: war add <name>, war remove <name>, war list");
		return;
	}
	const char *pSub = pResult->GetString(0);
	if(str_comp(pSub, "list") == 0)
	{
		if(g_Config.m_ClWarPlayers[0] == '\0')
			pClient->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "war", "War list is empty.");
		else
		{
			str_format(aBuf, sizeof(aBuf), "War players: %s", g_Config.m_ClWarPlayers);
			pClient->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "war", aBuf);
		}
		return;
	}
	...
	while(true)
	{
		...
		else
		{
			if(aNewList[0] != '\0')
				str_append(aNewList, ",");
			str_append(aNewList, aToken);
		}
	}
	if(str_comp(pSub, "add") == 0)
	{
		if(Found)
		{
			str_format(aBuf, sizeof(aBuf), "'%s' is already in war list.", pName);
			pClient->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "war", aBuf);
			return;
		}
		if(aNewList[0] != '\0')
			str_append(aNewList, ",");
		str_append(aNewList, pName);
		str_copy(g_Config.m_ClWarPlayers, aNewList, sizeof(g_Config.m_ClWarPlayers));
		str_format(aBuf, sizeof(aBuf), "Added '%s' to war list.", pName);
		pClient->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "war", aBuf);
	}
	else if(str_comp(pSub, "remove") == 0)
	{
		if(!Found)
		{
			str_format(aBuf, sizeof(aBuf), "'%s' is not in war list.", pName);
			pClient->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "war", aBuf);
			return;
		}
		str_copy(g_Config.m_ClWarPlayers, aNewList, sizeof(g_Config.m_ClWarPlayers));
		str_format(aBuf, sizeof(aBuf), "Removed '%s' from war list.", pName);
		pClient->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "war", aBuf);
	}
	else
	{
...
bool CGameClient::IsWarPlayer(const char *pName) const
{
	if(g_Config.m_ClWarPlayers[0] == '\0')
		return false;
	char aToken[32];
...
void CGameClient::SetLastChatMsg(int ClientId, const char *pMessage)
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS)
		return;
	str_copy(m_aaLastChatMsg[ClientId], pMessage);
	m_aLastChatTime[ClientId] = time_get();
}

const char *CGameClient::GetLastChatMsg(int ClientId, int64_t *pTime) const
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS)
		return nullptr;
	*pTime = m_aLastChatTime[ClientId];
	return m_aaLastChatMsg[ClientId];
}
